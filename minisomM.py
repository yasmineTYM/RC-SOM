...
from numpy import (array, unravel_index, nditer, linalg, random, subtract, max,
                   ...
                   einsum, prod, nan, sqrt, hstack, diff, argmin, multiply, where, 
                   nanmean, nansum)
...
som = MiniSom(100, 100, data.shape[1], sigma=1.5, resource_limit = 1, learning_rate=.7, activation_distance='euclidean',
              topology='hexagonal', neighborhood_function='gaussian', random_seed=10)
som.train(data,30000,verbose = True)
"""
...
class MiniSom(object):
    def __init__(self, x, y, input_len, resource_limit=None, sigma=1.0, learning_rate=0.5,
             decay_function=asymptotic_decay,
             neighborhood_function='gaussian', topology='rectangular',
             activation_distance='euclidean', random_seed=None, resources=None):
        ...
        if sigma >= x or sigma >= y: 
            warn('Sigma is too high for the dimension of the map')
        ...
        self._resource_limit_each = resource_limit
        self._resource_map = ones((x, y))
        self._resource_initialize() # intialize resource_map
    ...
    def _resource_initialize(self):
        if self._resource_limit_each:
            self._resource_map = ones(self._activation_map.shape) * self._resource_limit_each
    ...
    def winner(self, x):
        ...
        self._activate(x)
        if self._resource_limit_each:
            resource_adjuster = ones(self._activation_map.shape)
    #         _resource_map
            resource_adjuster[where(self._resource_map == 0)] = self._inf
            self._activation_map = self._activation_map * resource_adjuster
                 
            min_idx = unravel_index(self._activation_map.argmin(),
                                 self._activation_map.shape)
            
            if self._resource_map[min_idx] == 0:
                raise ValueError("resource already used up!")
            else:
                self._resource_map[min_idx] -= 1
        else:
            min_idx = unravel_index(self._activation_map.argmin(),
                                 self._activation_map.shape)
        return min_idx
    ...
    def train(self, data, num_iteration, interval, runEval=False,random_order=False, verbose=False):
        """Trains the SOM.
        ...
        for t, iteration in enumerate(iterations): 
            if t % len(data) == 0:
                print(f"\rworking on {batch_idx}th batch right now, updating resource, status: {self._resource_map.max(), self._resource_map.min()}")
                self._resource_initialize()
                batch_idx += 1
        ...
        #     # if runEval:
        #     #     if t%interval==0:
        #     #         e1 = self.quantization_error(data)
        #     #         e2 = self.topographic_error(data)
        #     #         error1.append(e1)
        #     #         error2.append(e2)
        #     # if t % len(data) == 0:
        #     #     print(f"\rworking on {batch_idx}th batch right now, updating resource, status: {self._resource_map.max(), self._resource_map.min()}")
        ...
        if verbose:
            print('\n quantization error:', self.quantization_error(data))
        # print('total length of error', len(error_list))
        # error1.append(self.quantization_error(data))
        # error2.append(self.topographic_error(data))
        # return error1, error2
        # return self.quantization_error(data), self.topographic_error(data)
    ...
    def evaluate(self, data):
        self._resource_initialize() # initialize resource map
        evaluation_res = {}
        for cnt, x in enumerate(data):
            w = som.winner(x)
            evaluation_res[cnt] = w
        self._resource_initialize()
